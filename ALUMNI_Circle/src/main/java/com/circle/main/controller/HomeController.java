...
import com.circle.main.helper.Message;
import com.circle.main.service.EmailService;
...
//			System.out.println("No principal");
		...
		model.addAttribute("title", "Home - alumni_360");
		return "home";
	}
	@GetMapping("/about")
	public String about(Model model, Principal principal) {
		...
		User user = userRepository.getUserByUserName(userName);
		model.addAttribute("user", user);
		model.addAttribute("title", "About - alumni_360");
		return "about";
	}
...
//			System.out.println("No principal");
	...
	public String signup(Model model) {
		...
		model.addAttribute("title", "Register - alumni_360");
	...
	public String login(Model model, HttpSession session) {
		...
		model.addAttribute("title", "Login - alumni_360");
	...
	public String sendOTPHandler(@RequestParam("email") String email, Model model, HttpSession session) {
		...
		String from = "jr446926@gmail.com";
		String to = email;
		String message = "<h2>Your OTP is here : <b>" + otp + "</b></h2>";
		String subject = "OTP From Alumni_360:circle of our alumni";
	...
	public String sendOTPToEmailHandler(@RequestParam("email") String email, Model model, HttpSession session) {
		...
		String to = email;
		String message = "<h2>Your OTP is here : <b>" + otp + "</b></h2>";
		String subject = "OTP From Alumni_360";
		boolean flag = this.emailService.sendEmail(message, subject, to, from);
		if(flag) {
			session.setAttribute("myotp", otp);
			session.setAttribute("email", email);
